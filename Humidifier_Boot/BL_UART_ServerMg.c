...
*/
{
  ...
  switch (progBoot.typeMessage)
  {
    ...
    case SEND_FRMWR_VERSION:
	  STUFF_BYTE( txBuffer->buffer, idx, LSB(BL_SW_VERSION));
      STUFF_BYTE( txBuffer->buffer, idx, MSB(BL_SW_VERSION));
      break;
  }
...
*/
{
  ...
  case CMD_FW_UPLOAD:
    ...
    progBoot.startAddress = tmpWord1.uword;
  ...
  case DATA_FW_UPLOAD:
    ...
    tmpWord1.byte[0] = rxBuffer->buffer[idx ++];
    tmpWord1.byte[1] = rxBuffer->buffer[idx ++];
    progBoot.address = tmpWord1.uword;

    progBoot.numDataBytesPack = rxBuffer->buffer[idx ++];

//    for (unsigned char i = 0; i < BYTES2WORDS(RequestDataBlockSize); ++ i) {
    for (i = 0; i < BYTES2WORDS(RequestDataBlockSize); ++ i) {
      tmpWord1.byte[0] = rxBuffer->buffer[idx ++];
      tmpWord1.byte[1] = rxBuffer->buffer[idx ++];
...
/**/
{
  ...
  case GET_DATA:
//    for(unsigned char i = 0; i < (progBoot.numDataBytesPack/2); i = i+2) {
